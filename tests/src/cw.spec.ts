...
const { gaia, setup, setupWasmClient, wasmd } = testutils;

// import { CwIbcCallbackExampleClient, CwIbcCallbackExampleQueryClient } from "./CwIbcCallbackExampleContract";
import {
  // assertPacketsFromA,
  // assertPacketsFromB,
  setupContracts,
...
test.before(async (t) => {
  console.log(cwIbcCallback.ordering, cwIbcCallback.version);

  const contracts = {
    // using arm64 optimizer
    cwIbcCallback: "cw_ibc_callback_example-aarch64.wasm",
  };
  codeIds = await setupContracts(contracts);
  t.pass();
});

test.serial("set up channel with cwIbcCallback contract", async (t) => {
  const cosmwasm = await setupWasmClient();

  // instantiate contract
  const cwIbcCallbackMsg = {
    default_timeout: 3600,
    gov_contract: cosmwasm.senderAddress,
    allowlist: [],
  };
  const { contractAddress: cwIbcCallbackAddr } =
    await cosmwasm.sign.instantiate(
      cosmwasm.senderAddress,
      codeIds.cwIbcCallback,
      cwIbcCallbackMsg,
      "IBCCLLBCK",
      "auto"
    );
  t.truthy(cwIbcCallbackAddr);

  const { ibcPortId: wasmPort } = await cosmwasm.sign.getContract(
    cwIbcCallbackAddr
  );
  ...
  await link.createChannel(
    ...
    gaia.ics20Port,
    wasmPort,
    cwIbcCallback.ordering,
    cwIbcCallback.version
  );
});
...
//     ],
//   };
//   const { contractAddress: cwIbcCallbackAddr } = await cosmwasm.sign.instantiate(
//     cosmwasm.senderAddress,
//     codeIds.ics20,
//     ics20Msg,
//     "ICSX",
...
//   const { ibcPortId: wasmPort } = await cosmwasm.sign.getContract(cwIbcCallbackAddr);
//   console.log(`Ibc Port: ${wasmPort}`);
//   assert(wasmPort);
...
//   // (dest chain is wasmd)
//   const sendMsg = sendTokens(cwIbcCallbackAddr, "456789000", {
